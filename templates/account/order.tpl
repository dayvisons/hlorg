<div class="page-account container page">
    <div class="headerBox-Page row">
        <div class="col-md-8 col-sm-8 col-xs-12">
            <h2>{{ 'Orden {1}' | translate(order.number) }}</h2>
        </div>
    ...
    <div class="row">
        <div class="details-box col-md-4 col-sm-4 col-xs-12">
            {% if log_entry %}
                <h3>{{ 'Estado actual del envío' | translate }}:</h3><div id="current_shipping_state">{{ log_entry }}</div><br>
            {% endif %}
        ...
        <div class="order-box col-md-8 col-sm-8 col-xs-12">
            ...
            <div class="order-actions">
                {% if order.pending %}
                    <a class="product-related-button" href="{{ order.checkout_url | add_param('ref', 'orders_details') }}" target="_blank">{{ 'Realizar el pago >' | translate }}</a>
                {% endif %}
            </div>
        </div>
    </div>
